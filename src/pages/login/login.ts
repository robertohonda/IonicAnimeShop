...
export class LoginPage {
  ...
  user: User = {name: '', username: '',email: '', password: '', isLogged:false, isAdmin: false};
  ...
  singInComands(){
    this.afAuth.auth.signInWithEmailAndPassword(this.userProv.currentUser.email, this.userProv.currentUser.password).then(data=>{
      this.navCtrl.setRoot(HomePage);
        ...
        this.userProv.currentUser = null;
  ...
  signIn(user: User){
    ...
    try{
      ...
      this.db.database.ref('/users/' + user.username).once('value').then( snapshot => {
        ...
        this.userProv.currentUser = userFound; // atualizar usuario corrente 
      }).then(data => {
        this.singInComands();
      }).catch(error =>{
        this.alert.showAlert('Error!', error.message);
